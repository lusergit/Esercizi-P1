...
doppialista* match(nodo* L, int *P, int dimP){
	nodo * current = L;
	...
	return new doppialista(lista_match.inizio,resto.inizio);
}
...
int main(){
	...
	int *A = new int[k];
	leggi(A,k);
	doppialista* L1 = match(L,A,k);
	stampa(L1->listamatch);
	stampa(L1->resto);
	return 0;
}